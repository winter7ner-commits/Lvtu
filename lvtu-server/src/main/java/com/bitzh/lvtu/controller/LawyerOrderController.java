...
public class LawyerOrderController {
    ...
    public ApiResponse<LawyerProfile> getLawyerProfileByUserId(@PathVariable @NotNull Long userId) {
        LawyerProfile lawyerProfile = lawyerProfileMapper.selectByUserId(userId);
        if (lawyerProfile == null) {
            throw new BusinessException("当前用户不是可接单律师");
        }
        return ApiResponse.success(lawyerProfile);
    }
